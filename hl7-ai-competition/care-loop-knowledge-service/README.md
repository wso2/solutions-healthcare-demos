...
- `corpus/feature_glossary.md`, a hand-written glossary of the 11 heart-risk features (of which the model scores 9).

Licensed guideline PDFs can be dropped into the gitignored `corpus/raw/` and registered in `scripts/sources.py` as `kind="pdf"`.

## Build the vector store

The Chroma store is a build artifact (like the heart-risk ONNX model) and is not committed. Build it with:

```sh
make ingest    # fetches/snapshots the corpus, embeds it, writes data/chroma/
```

`make ingest` needs `KNOWLEDGE_OPENAI_API_KEY` (or `OPENAI_API_KEY`) to embed with `text-embedding-3-small`. For a keyless, offline build use a local embedder:
...
In the stack it is published on host port 8006 (container port 8000; see the root `docker-compose.yml`).
