...
// Forces the healthkit simulator's vitals-forward cron to run immediately instead of waiting for
// its own schedule, so a demo trigger drives the real vitals -> ML -> agent pipeline (server-side
// to avoid a browser CORS request to another container/port).
