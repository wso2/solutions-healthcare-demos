...
// All three MCP servers (FHIR, knowledge, PubMed) are hard startup dependencies: the docker
// compose stack starts them together with healthchecks + depends_on, so `check` here fails fast
// on a genuinely broken toolkit rather than the service booting into a half-grounded state.
//
// httpVersion is pinned to 1.1: the client defaults to HTTP/2, which over cleartext sends an
// h2c upgrade the Python-based MCP servers (uvicorn) mishandle, dropping the request body and
// returning 400 on initialize.
...
// The interview agent gets only the knowledge toolkit: every patient fact it needs arrives
// prefilled in the turn request, so it never touches FHIR, keeping per-turn latency low.
...
// Newer Claude snapshots (4.5-era) sometimes wrap the required JSON in a markdown code fence and/or
// prepend a sentence of reasoning before it, despite every prompt saying not to. Rather than rely on
// prompt wording alone, take the outermost {...} span so a fenced or prose-prefixed answer still parses.
...
// before answering; the default 60s listener idle timeout was cutting the HTTP/2 stream ("Stream
// timed out") out from under it before the response was ready, even after the caller's own client
// timeout (care-loop-analysis-service/clients.bal) was raised.
...
service /conversation on sharedListener {
    ...
    // One turn of the adaptive check-in: extract slots from the latest answer and pick the next
    // question (or finish). Stateless - all history rides in the request; memory is isolated per
    // patient only so the agent's own trace groups cleanly.
